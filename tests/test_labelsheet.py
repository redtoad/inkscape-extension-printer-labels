...
def test_get_size_different_template():
...
def test_set_namedview_basic(mock_init):
...
@patch.object(LabelSheet, "__init__", return_value=None)
def test_set_namedview_with_shapes(mock_init):
...
@patch.object(LabelSheet, "__init__", return_value=None)
def test_set_namedview_with_grid(mock_init):
...
def test_labels_list_structure():
...
def test_label_avery_zweckform():
